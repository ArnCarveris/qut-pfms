...
% in the xy plane for the required time to reach the specified waypoint.
% The model considers the maximum turning rate of the UAS and utilises P 
...
%% Script Settings
% Constant
g = 9.80665;                % Nominal acceleration of gravity at sea level (m/s^2)

% Control Settings
prediction_horizon = 10000; % Prediction Horizon
sampling_interval = 1;      % Sampling time
K = 4;                      % Proportional control gain
prox = 25;                  % Assign simulation prox required to complete navigation
tolerance = 1E-5;           % Control Tolerance

% UAS Settings
init_loc_UAS = [0, 0];      % Initial UAS location
init_head_UAS = pi/2;       % Initial UAS angle of motion (radians)
...
% Waypoint Data
waypoint = [ 2000, -3000 ];  % Initial target location
...
while (~at_waypoint)   
    ...
    if a_n ~= 0        
        ...
        d_theta = (vel_UAS/(1/sampling_interval))/r;
        ...
        d_position =  [(d_length*cos(alpha+(d_theta/2))) (d_length*sin(alpha+(d_theta/2)))];         
        ...
        d_position = [vel_UAS*cos(alpha)*sampling_interval vel_UAS*sin(alpha)*sampling_interval];        
...
%/*************************************
figure(1)  
plot(loc_UAS_v(:,1), loc_UAS_v(:,2),'r')
hold on
plot(waypoint(1), waypoint(2), 'bx')
plot(init_loc_UAS(1), init_loc_UAS(2), 'kx')
